...
class requestsAPP(customtkinter.CTk):
    user_input = ""
    historique_conversation = [{'role': 'system', 'content': self.user_consigne_text}]
    temperature = 1
    user_max_token = 2048
    ...
    def __init__(self):
        ...
        #partie condition
        self.consigne_frame = customtkinter.CTkFrame(self, width=800, corner_radius=0) 
        self.consigne_frame.grid(row=1, column=0)

        self.user_consigne = customtkinter.CTkEntry(master=self.text_frame, height=30, width=200)
        self.user_consigne.grid()

        self.consigne_button = customtkinter.CTkButton(master=self.consigne_frame, text ="Apply", command=self.appy_consigne)
        self.consigne_button.grid()
    ...
    def appy_consigne(self):
        c =self.user_consigne.get()
        self.user_consigne_text = c
        print(self.user_consigne_text)
